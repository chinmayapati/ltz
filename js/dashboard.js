$(document).ready(function () {
    ...
    function createToast(notification) {
        ...
        setTimeout(function() { $('.toast').html('').removeClass('show'); }, 3000);
    }
        ...
        if(keycode == '13'){
            ...
            if(searchString!=""){
                ...
                $.ajax({
                    ...
                    statusCode: {
                        500: function () { createToast("Internal Server Error"); }
                    },
                    success: function (response) {
                        var s="";
                        var result = jQuery.parseJSON(response);
                        var host = window.location.hostname;
                        var upload_path = "/Projects/ltz/uploads/";
                        ...
                        if(result.length!=0) {
                            for (var i=0; i<result.length; i++) {								
                                ...
                                s += "<td><a href='http://"+ address + result[i]['file_name'] + "'style='cursor: hand;'><span class='glyphicon glyphicon-download-alt' aria-hidden='true'></span></a></td></tr>";
                            }
    ...
    $('#profile-btn').click(function () {
        ...
        if(fname!="" && lname!=""){
            $.ajax({
                type: "get",
                url: "profile.php",
                data: {type: "profile", fname: fname, lname: lname},
                statusCode: {
                    500: function () {createToast("Internal Server Error");}
                },
                success: function (response) {
                    ...
                    else {createToast("Please Try After Sometime");}
                },
                error: function (response) {
    ...
    $('#changepwd-btn').click(function () {
        ...
        if(neww==cnf){
            $.ajax({
                type: "get",
                url: "profile.php",
                data: {type: "password", curr: curr, new: neww, cnf: cnf},
                statusCode: {
                    500: function () { createToast("Internal Server Error"); }
                },
                success: function (response) {
