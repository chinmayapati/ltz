...
class Database
{
    private $host = "localhost", $username = "root", $password = "toor";
	...
	foreach($k as $i) $searchStr .= $i."%";
        ...
        $sql = "SELECT file_name,times_dw,uploaded,link FROM `files` WHERE file_name LIKE '$searchStr' ORDER BY uploaded DESC;";// LIMIT 0, 7 ;";
        $res = mysqli_query($this->link, $sql);
        if( $res ){
            $a = [];
